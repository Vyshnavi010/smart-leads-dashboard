...
export const getLeads = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (source) {
      filterQuery.source = source as string;
    }
...
export const exportCSV = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const filterQuery: any = {};
    if (status) filterQuery.status = status as string;
    if (source) filterQuery.source = source as string;
    if (search) {
      const searchStr = search as string;
      filterQuery.$or = [
        { name: { $regex: searchStr, $options: 'i' } },
    ...
    const leads = await Lead.find(filterQuery).sort(sortOptions);
